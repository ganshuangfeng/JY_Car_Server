...
BEGIN
	...
	if not @only_one_level and @my_level='GamePartnerSenior' then
		-- 逐级循环，直到没有
    ...
    left join club_info c on a.id = c.id 
		...
		inner join (
					select player_id,channel_type,min(create_time) create_time 
					from (select * from player_pay_order where order_status='complete' and create_time>=@start_time union all select * from player_pay_order_log where order_status='complete' and create_time>=@start_time) a 
					group by player_id
