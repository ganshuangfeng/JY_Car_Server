...
BEGIN
    ...
    left join club_info c on a.id = c.id 
		...
		left join (
					...
					from (select * from player_pay_order where order_status='complete' union all select * from player_pay_order_log where order_status='complete') a 
					where product_id='8' and create_time >= @start_time  group by player_id
