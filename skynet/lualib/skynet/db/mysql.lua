...
function _M.query(self, query)
    ...
    -- by lyx 2021-1-12 : 发生 'packet size too big'  没有设 _ret.errno，这里补上
    local _ret = sockchannel:request( querypacket, self.query_resp )
    if _ret.badresult then
        if not _ret.errno then
            _ret.errno = 999999
        end
    end
    return _ret    
