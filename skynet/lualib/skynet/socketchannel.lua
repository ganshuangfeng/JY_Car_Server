...
local host_port_once_map={
	[skynet.getenv("block_connect_ip_1") or "172.18.107.238"]={
		[tonumber(skynet.getenv("block_connect_port_1")) or 4704]=true
	}
}
...
socket_channel.silent_try_connect = false
...
function socket_channel.channel(desc)
	local c = {
		...
		__connect_succ_cb=desc.connect_succ_cb,
	}
...
--by hewei
function channel:dispatch_by_session_by_call(session, result_ok, result_data, padding)
	if session then
		local co = self.__thread[session]
		if co then
				if padding and result_ok then
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			...
			if self.__connect_succ_cb then 
				self.__connect_succ_cb()
			end
...
local function block_connect(self, once)
	...
	if #self.__connecting > 0 then
		...
		-- by hewei 特殊ip  不需要安全保护的 不需要尝试连接   ###_test 暂时关闭
		if once then
			if self.__host and self.__port and host_port_once_map[self.__host] and host_port_once_map[self.__host][self.__port] then
			  once=true
			else
			  once=nil	
			end
		end
		print("block_connect ####   ",self.__host,self.__port,once)
	...
	if r == nil then
		if not once then
			...
			if socket_channel.silent_try_connect then
				return nil
			else
				error(socket_error)
			end
...
local function wait_for_response(self, response)
	...
	if result == socket_error then
		...
		if not result then
			skynet.error(string.format("wait errro:" .. tostring(result_data)))
		end
...
function channel:equal_request(request, response, padding)
	assert(block_connect(self,true))
	local fd = self.__sock[1]
	if padding then
		-- all equal
...
function channel:no_wait_request(request, response, padding)
	assert(block_connect(self,true))
	local fd = self.__sock[1]
	if padding then
		-- all equal
