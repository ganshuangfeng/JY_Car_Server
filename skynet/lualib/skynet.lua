...
-- by lyx 
function skynet.concat_vars(...)
	local t = table.pack(...)
	for i=1,t.n do 
		if "string" == type(t[i]) then
			t[i] = "'" .. t[i] .. "'"
		else
			t[i] = tostring(t[i])
		end
...
function skynet.timeout(ti, func)
	ti = math.min( ti , 3153600000 )  --- 最大一年
...
skynet.jy_pack=assert(c.jy_pack)
skynet.get_jy_id=assert(c.get_jy_id)
...
local function _debug_ret(_tmp1,...)
	if _tmp1 == "__debug_call_error__" then
		print("===== call error,stack =====",debug.traceback())
	end
...
function skynet.call(addr, typename, ...)
	...
	--return _debug_ret(p.unpack(yield_call(addr, session)))
end
function skynet.rawcall(addr, typename, msg, sz)
	...
	if not session then
		error("call to invalid address:" .. skynet.concat_vars(addr, typename, msg, sz))
	end
