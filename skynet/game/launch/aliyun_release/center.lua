...
-- 商城服务器
--local shoping_server = "https://mall-webapp-user.jyhd919.cn"
local shoping_server = "https://mall-webapp-user.jyhd919.cn/jyhd/jyddz"

--local shoping_api_server = "http://mall-server-user.jyhd919.cn"
local shoping_api_server = "http://mall-server-user.jyhd919.cn/jyhd/jyddz"

-- 充值服务器
--local payment_server = "http://es-caller.jyhd919.cn"
local payment_server = "http://es-caller.jyhd919.cn/jyhd/jyddz"
...
local configs =
{
	...
	launch_ok_shell = "/home/jy/skynet/launch_ok.sh",

	-- 服务器名字：客户端用于区分正式服 还是 其他用途服务器。 zs 正式； cs 测试
	server_name = "zs", -- 正式
	...
	-- 允许的 gm 命令(逗号隔开，结尾必须包含逗号)
	-- "*" 或 nil 表示 允许所有； "off" 表示禁止所有
	--  为了金融安全，不能允许 诸如 money 之类的命令！
	allow_gm_command = "off",
	...
	get_share_url = "https://s.jyhd919.cn/jyhd/jyddz/common/generateUserRecommendQrCode2?playerId=@userId@",
	--get_share_url = "https://gjhhrht.jyhd919.cn/jyhd/common/generateUserRecommendQrCode2?playerId=@userId@",

	-- 1元话费url
	pay_phone_tariffe_url = shoping_api_server .. "/OrderTransactor.goodsOrderPlaceForShoppingGoldPay1.command",

	-- 1元话费商品id
	pay_phone_tariffe_goodsid="89133",
...
--	templateCode_bind_phone = "SMS_136171608",
	...
	-- 手机短信登录要求 图片验证
	sms_login_require_pic_vcode = true,

	-- 一元话费的短信通知
	--pay_phone_tariffe_note = "您的1元话费已到账，祝您游戏愉快，比赛赢大奖！",
	...
	monitor_email_time = 30,

	-- 不真正发邮件，仅仅输出日志（测试用）
	--monitor_mock_email = true,

	-- 接收报警数据 的 邮件地址（支持多个，逗号隔开）
	monitor_recieve_email = "cool.ryu@qq.com,695223392@qq.com,421951953@qq.com,24090841@qq.com,949652665@qq.com,897106850@qq.com,83046971@qq.com,liangpai@163.com",

	-- ★ 非正式配置。注意： 正式发布要去掉！！！

	--is_pay_test = true,
	--debug_player_prefix = "R",
    ...
    --
    lucky_id_1 = "102825055",
    lucky_id_2 = "102852268",

    ---- 幸运号 ******************************* ↑↑↑

    --- 财神模式的返奖率
    zjd_csmode_zj_power = 92,
	...
	free_tuoguan_count = 4000,

	tuoguan_max_count = 8000,
	...
	-- 烂牌 的情况下 抢地主的概率
	freestyle_game_down_bad_pai_dizhu = 20,
	freestyle_game_free_bad_pai_dizhu = 1,
	freestyle_game_up_bad_pai_dizhu = 2,
	match_game_down_bad_pai_dizhu = 20,
...
local nodes =
{
	...
	{
		-- 数据服务配置
		mysql_host = "172.18.107.235",
		mysql_dbname = "jygame",
		mysql_port = 2356,
		mysql_user = "game",
		mysql_pwd = "jiNgyUddZ568",
...
local services =
{
	...
	{	-- 数据
		launch = "data_service/data_service_old", -- 正式服 暂不启用优化
