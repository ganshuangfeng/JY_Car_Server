...
function PUBLIC.channels.youke.verify(_login_data)
	...
	--dump(_login_data,"xxxxxxxxxxxxxxxxxdddddddddd youke verify:")

	local _login_id = _login_data.login_id or ""

	local _user = PUBLIC.get_player_verify_data(_login_data.platform,"youke",_login_id)
	...
	if _user then
		...
		if "" == _login_id then
			_login_id = skynet.random_str(30)
		end

		return true,{
			login_id=_login_id,
		},{
			name="游客"..  math.random(134,9999),
			sex=1,
		}
