...
PUBLIC.channels.wechat = PUBLIC.channels.wechat or {}
local LF = PUBLIC.channels.wechat
...
function LF.getUserInfo( authArgs )
	...
	if ok then
		...
		dump(content,"channel wechat getUserInfo error:")
...
--]]
function LF.verify(_login_data)
	...
	local _appid,_appkey

	local status, retArgs

	if _login_data.channel_args then
		status, retArgs = pcall( cjson.decode, _login_data.channel_args )
		if not status then
		...
		if retArgs.appid and _sdk_cfg.appids[retArgs.appid] then
			_appid = retArgs.appid
			_appkey = _sdk_cfg.appids[retArgs.appid]
		end
	end

	-- 客户端没给出 appid ， 用默认的
	if not _appid then
		_appid = _sdk_cfg.channels.nor.wx_appid
		_appkey = _sdk_cfg.channels.nor.wx_appkey
	end
	...
	if _login_data.login_id then
		...
		local _user = PUBLIC.get_player_verify_data(_login_data.platform,"wechat",_login_data.login_id)
		...
		if not _user then
			...
			if ok then
				...
				local ok, user_info = LF.getUserInfo( retArgs )
				if( not ok ) then
	...
	elseif _login_data.channel_args then
		...
		if ok then
			...
			dump({wechat_content=content,url=url_to},"wechat verify error 300:")
			return nil,2154,content
		end
...
function LF.after_verify(is_new_user,player_id,new_verify_data,new_user_data,_login_data)
	...
	skynet.call(DATA.service_config.data_service,"lua","add_player_openid",
			player_id,
			retArgs.appid or _sdk_cfg.channels.nor.wx_appid,
