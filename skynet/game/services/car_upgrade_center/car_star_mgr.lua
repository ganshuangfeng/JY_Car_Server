...
local LD = base.LocalData("car_star_mgr",{
...
local LF = base.LocalFunc("car_star_mgr")
...
function LF.refresh_config(_config)
    ...
    -- rule 表 内容： rule id => 数组：{star=,max_level=,}
    local _star_rules = {}
...
function LF.get_car_cfg(_car_id)
    return LD.upgrade_config[_car_id]
end
...
--[[
    ...
    或 nil,code
--]]
function CMD.get_car_star_spend(_car_id,_star)
    ...
    return basefunc.safe_keys_cb(LD.upgrade_spend_cache,_cb,_car_id,_star)
...
--]]
function CMD.get_car_star_skill_change(_car_id,_star)
    local _d = LD.upgrade_config[_car_id]
    if not _d then
        return nil
    end
    ...
    return basefunc.safe_keys_cb(LD.upgrade_skill_cache,_cb,_car_id,_star)    
end
