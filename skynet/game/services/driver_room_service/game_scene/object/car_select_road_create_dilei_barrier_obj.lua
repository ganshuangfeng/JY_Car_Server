...
local C = basefunc.create_hot_class("car_select_road_create_dilei_barrier_obj" , "object_class_base" )
...
function C:ctor(_d  , _config)
	...
	------
	self.barrier_id = 1 -- _config.barrier_id
	----- 选择的道路数量
...
function C.msg_deal:player_op_msg(_op_data)
	if _op_data.base_op_type == self.op_type then
		...
		local dilei_barrier = PUBLIC.create_map_barrier( self.d , self.barrier_id , target_road_id , process_id , self.owner , self.is_clear_enemy_barrier )

		if dilei_barrier then
			local at = DATA.car_prop_lib.get_car_prop( self.owner , "at" )
			dilei_barrier.dilei_fix_damage = math.floor(at * self.at_factor / 100)
		end
