...
DATA.car_hongdeng_skill_protect = {}
local D = DATA.car_hongdeng_skill_protect
...
function D.new( _d , _config , _data )
	...
	function C:deal_other_msg()
		----- 当 
		self.msg_deal["player_canot_nor_op"] = function(_self , _arg )
			
			if self.owner.seat_num == _arg.seat_num then
				self.work_num = self.work_num - 1
				print("xxx--------------car_hongdeng_skill ,  ", self.work_num )
