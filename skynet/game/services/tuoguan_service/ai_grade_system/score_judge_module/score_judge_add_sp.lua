...
local C = basefunc.create_hot_class("score_judge_add_sp" )
C.msg_deal = {}
...
function C:ctor( _config)
	...
	--- 自己关心的 效果
	self.deal_effect = PROTECT.effect_type.add_sp
...
function C.msg_deal:deal_score_judge( _effect_score_data , _game_data , _effect , _tar_seat , _effect_value )
	...
	print("xxx-----------score_judge_add_sp")

	

	---- 确定基础分 (一个单位量的价值) ，法拉利的加速度的分数要高于 坦克
	local score = 10

	local tar_game_data = _game_data[_tar_seat]

	if tar_game_data.car_id == 1 then
		score = score * 2
	end
