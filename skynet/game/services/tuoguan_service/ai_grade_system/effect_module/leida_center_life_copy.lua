...
local C = basefunc.create_hot_class("leida_center_life_copy" )
C.msg_deal = {}
...
function C.msg_deal:on_stay_road( _effect_data , _game_data , _type_id , _gl , _other_data)
	...
	print("xxxx-------------effect_module_life_copy_add_hp____on_stay_road")
	...
	effect_value = effect_value - my_car_hp

	local enemy_effect_value = self.add_value_bfb / 100 * award_num * _game_data[_game_data.enemy_seat].car_max_hp 
	local enemy_car_hp = _game_data[_game_data.enemy_seat].car_hp
	enemy_effect_value = enemy_effect_value - enemy_car_hp
