...
local C = basefunc.create_hot_class("road_falali_nor_big_skill" )
C.msg_deal = {}
...
function C.msg_deal:on_stay_road( _effect_data , _game_data , _type_id , _gl , _other_data )
	...
	for i=1 , step_len do
		...
		if now_road == PROTECT.pos_to_road_id(_game_data , _game_data[_game_data.enemy_seat].car_pos) then
			print("攻击了")
			...
			_effect_data[#_effect_data + 1] = { effect = PROTECT.effect_type.add_hp , tar_seat = _game_data.enemy_seat ,  value = effect_value , gl =  move_in_gl * self.gailv_bfb / 100 }
		end
	end
