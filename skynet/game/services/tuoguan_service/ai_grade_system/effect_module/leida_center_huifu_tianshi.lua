...
local C = basefunc.create_hot_class("leida_center_huifu_tianshi" )
C.msg_deal = {}
...
function C.msg_deal:on_stay_road( _effect_data , _game_data , _type_id , _gl , _other_data)
	...
	print("xxxx-------------effect_module_add_hp____on_stay_road")

	local my_car_max_hp = _game_data[_game_data.my_seat].car_max_hp
	local enemy_car_max_hp = _game_data[_game_data.enemy_seat].car_max_hp

	local effect_value = self.add_value_bfb / 100 * award_num * my_car_max_hp
	local enemy_effect_value = self.add_value_bfb / 100 * award_num * enemy_car_max_hp

	local my_car_hp = _game_data[_game_data.my_seat].car_hp
	local enemy_car_hp = _game_data[_game_data.enemy_seat].car_hp


	if my_car_hp + effect_value > my_car_max_hp then
		effect_value = my_car_max_hp - my_car_hp
	end
	if enemy_car_hp + enemy_effect_value > enemy_car_max_hp then
		effect_value = enemy_car_max_hp - enemy_car_hp
	end

	_effect_data[#_effect_data + 1] = { effect = PROTECT.effect_type.add_hp , tar_seat = _game_data.my_seat ,  value = effect_value , gl = _gl }
	_effect_data[#_effect_data + 1] = { effect = PROTECT.effect_type.add_hp , tar_seat = _game_data.enemy_seat ,  value = enemy_effect_value , gl = _gl }
