...
local C = basefunc.create_hot_class("road_nor_add_at" )
C.msg_deal = {}
...
function C.msg_deal:on_stay_road( _effect_data , _game_data , _type_id , _gl , _other_data )
	...
	print("xxxx-------------effect_module_add_at____on_stay_road")

	local effect_value = self.add_value_bfb /100 * award_num * _game_data[_game_data.my_seat].car_base_at 

	_effect_data[#_effect_data + 1] = { effect = PROTECT.effect_type.add_at , tar_seat = _game_data.my_seat , value = effect_value , gl = _gl }
