...
function client:on_login(data)
	...
	if data.result == 0 then
		...
		self:delay_send_request("mjfg_req_game_list", {},handler(self,self.on_respone_reg_game_list),2)
	else
...
function client:on_respone_reg_game_list(data)
	...
	self:delay_send_request("mjfg_signup", {id = 2},handler(self,self.on_sign_result))
...
function client.msg:mjfg_permit_msg(_name,_data)
