...
package.path = "lualib/?.lua;game/protocol/?.lua;game/common/?.lua;./game/test/?.lua"
require "printfunc"
...
-- 每秒连入用户
local COUNT_PER_SECOND = 100

-- 总用户数量
local MAX_COUNT = 500000000000000
...
function client:update(dt)
	...
	if self.is_login then
		...
		-- 登录 n 秒后退出
		self.login_dur_time = (self.login_dur_time or 0) + dt
		if self.login_dur_time >= 1.1 then
			self:delay_send_request("player_quit", {})
		end
	end
...
local clients = baselib.list.new()
...
while true do
	...
	-- 每次创建 一部分
	if _online_count < MAX_COUNT then
	...
	-- 已经创建对象的 update
	for _it,client in clients:pairs() do 
		if client.fd then
