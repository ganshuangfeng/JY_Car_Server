...
local SERVER = "192.168.10.26:5001"
...
function client:on_login(data)
	...
	if data.result == 0 then
		...
		self:delay_send_request("nor_mg_signup", {id = 10, watch_ad = 1},handler(self,self.on_respone_nor_mg_signup),2)
	else
...
function client:on_respone_nor_mg_signup(data)

	if data.result ~= 0 then
		dump(data,"on_respone_fsg_3d_signup_____")
		return
	end
...
function client:logic_update()
	
	--if self.fsg_data then
	--
	--	if self.frame_going then
	--		return
	--	end
	--
	--
	...
	--	d.shoot = {}
	--	-- 发射子弹
	--	if #self.fsg_data.bullets < 40 then
	--
	--		local n = math.random( 4 , 10 )
	--		for i=1,n do
	--			table.insert(d.shoot,{
	--				index = 1,
	--				seat_num = 1,
	--			})
	--		end
	...
	--
	--	-- 碰撞鱼
	--	if #self.fsg_data.bullets > 1 and #self.fsg_data.fishs > 1 then
	--		d.boom = {}
	--
	--		local bn = #self.fsg_data.bullets
	--
	--		local brn = math.random(3,6)
	--		bn = math.min(brn,bn)
	--
	--		for i=1,bn do
	--
	--			local bi = table.remove(self.fsg_data.bullets,1)
	--
	--			local fs = {}
	--
	--			local fn = #self.fsg_data.fishs
	--
	--			for ii=1,3 do
	--				local fi = math.random( fn )
	--				table.insert(fs,self.fsg_data.fishs[fi])
	--			end
	--
	--			table.insert(d.boom,{
	--				id = bi,
	...
	--	end
	--
	--	self:send_request("nor_fishing_3d_nor_frame_data_test", {
	--		data = d
	--	},handler(self,self.on_respone_fsg_3d_frame))
	--
	--	self.frame_going = true
	--end

end


function client:on_respone_fsg_3d_frame(data)

	-- dump(data,"fsg_3d_frame")

	local fd = data.data

	if not fd then

		dump(data,"eeeeeeeeeeee")
		return
	end

	-- 加鱼
	if fd.fish_group then
		
		if #self.fsg_data.fishs > 40 then
			self.fsg_data.fishs = {}
		end

		for i,v in ipairs(fd.fish_group) do
			table.insert(self.fsg_data.fishs,v.id)
		end
	...
	if fd.shoot then
		...
		for i,v in ipairs(fd.shoot) do
			if v.id > 0 then
				table.insert(self.fsg_data.bullets,v.id)
			end
		end
	...
	-- local ass = fd.assets[1].value

	-- self.output_data = self.output_data or {
	-- 	time = 0,
	-- 	money = ass,
	-- }

	-- if ct > self.output_data.time then

	-- 	if self.output_data.money ~= ass then
	-- 		self.output_data.money = ass
	-- 		print(string.format(
	-- 				"money:%s"
	-- 				,ass
	-- 			))
	...
	-- 	self.output_time = ct + math.random(8,12)

	-- end

	self.frame_going = nil
...
while true do
	...
	if #clients < MAX_COUNT then
		for i=1,math.max(1,COUNT_PER_SECOND * _dt) do
			...
			if not _client then 
				_exit_create = true
				print("create client error will exit create")
				break 
			end
			...
			if _client:connect() then
				...
				print("client connect error will exit create")
				_exit_create = true
				break
			end
